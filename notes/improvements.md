...
> **このファイルは開発中の作業ドキュメント (日本語可)。リリース時に `notes/` ごと削除する。**
> `docs/` は公開ツリー (英語専用、将来 GitHub Pages のソース)。恒久的に必要な知見は削除前に `docs/` か `.claude/rules/` に移す。
...
旧 #1〜#19 (カテゴリ A〜E) は 2026-06-11 に消し込み済み。経緯と結論は git log のこのファイルの履歴と各コミットを一次ソースとする。残すべき知見は本ファイル末尾の「引き継ぎ知見」と `docs/dev/binary-updates.md` に移送済み。
...
| F2  | F   | 更新手順の整備 (install.md + 更新確認ボタン) | 中   | 未着手   | 方針 (c) 確定済み: 手動 DL + Watch/RSS 案内 + 手動チェック |
...
**案**: `docs/licenses.md` (英語) を公開ツリーに新設。対象: ffmpeg (LGPL 2.1+) / llama.cpp (MIT) / whisper.cpp (MIT) / Gemma (Gemma Terms) / Qwen (Apache 2.0) / Silero VAD。`docs/dev/binary-updates.md` §5 の表をベースに利用者向けに書く。配布物としての表記義務 (LGPL の動的利用 + 別バイナリDL形態) の確認もここで。
...
├── privacy.md          # 新規: README の Privacy 節を独立ページ化 (最大の差別化点)
├── licenses.md         # F8 で作成済み
└── dev/
    ├── architecture.md     # 新規: 構造説明を一般向けに
    ├── binary-updates.md   # 既存
    └── releasing.md        # 新規: リリース手順 + 「引き継ぎ知見」の CI/リリース節を英訳して吸収
```
...
リリース時、CI/リリース運用の節は `docs/dev/releasing.md` (F9) に英訳して移す。バックログは要望が出たら個別に起票する。
...
- Settings の「再ダウンロード推奨」バナー (`docs/dev/binary-updates.md` §7 にも記載)
...
- 完了項目は次の節目で表から消し込んでよい。**消し込む前に、将来必要な知見を「引き継ぎ知見」か恒久ドキュメント (`docs/` / `.claude/rules/`) に移す**。議論経緯は git log を一次ソースとする。
- 項目を追加するときは、**進捗サマリー表と新規セクションを同じコミットで追加**する。片側更新にしない。
- このファイルと `notes/` はリリース時に削除する。削除前チェック: 「引き継ぎ知見」の移送完了・全項目が完了または明示的な保留/バックログ化。
