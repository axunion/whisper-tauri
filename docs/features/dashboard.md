...
| 推奨バッジ | `recommended: true` のモデルに「おすすめ」バッジを表示 |
| 推奨理由 | PC のスペック（RAM・CPU）に基づく自動判定であることを補足表示 |
| 操作 | 未ダウンロードモデルのダウンロード開始 |
| 現在のモデル | 使用中モデルをハイライト |

**補足**: 推奨モデルは Step 3 のシステム推奨ロジックにより、ユーザーの PC スペックに応じて自動的に決定される。
...
- [x] `pnpm test` でテストが通る
- [x] サイドバー型レイアウトが表示される
- [x] ダッシュボードが初期画面として表示される
- [x] サイドバーから各画面に遷移できる
- [x] 文字起こし画面が正常に動作する
- [x] クイックアクションから各機能に遷移できる
- [x] 履歴一覧が表示される（または空表示）
- [x] モデル一覧が表示される
...
- **カテゴリラベル廃止**: サイドバーのセクション分け（メイン/設定/開発）を廃止し、フラットなメニュー構成に変更。Dev メニューのみ区切り線で分離
- **サイドバー折りたたみ**: `collapsible="icon"` を採用。閉じるとアイコンのみ残るスタイル。トグルボタンはサイドバー右上に配置
- **ヘッダー削除**: メインコンテンツ側のヘッダーを廃止。SidebarTrigger はサイドバー内に統合
- **ページ遅延ロード**: Transcription, Settings, DevMenu は `lazy()` で遅延ロード
- **solid-ui Sidebar**: デスクトップアプリのため、モバイル対応（Sheet）、Tooltip、Skeleton、TextField、Separator の依存を除外した簡易版を使用
- **ダークモード**: CSS変数は定義済みだが、切り替え機能は未実装
